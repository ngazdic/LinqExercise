...
namespace LinqExercise
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            numbers.OrderBy(x => x).Where(x => x < numbers.Length - 5).ToList().ForEach(x => Console.WriteLine(x));
            ...
            employees.Where(x => x.Age > 26).OrderBy(x => x.Age).OrderBy(x => x.FirstName).ToList().ForEach(x => Console.WriteLine($"{x.Age}  {x.FullName}"));
            ...
            employees = employees.Append(new Employee { Age = 31, FirstName = "Nick", LastName = "Gazdic" }).ToList();
